...
def login(request):
    ...
    if session_id and session_id != "vbs":
        ...
        url = f"{server}/login/"
        res = requests.post(
            url, json={"username": "myeachtra", "password": "mQFbxFf9Cx3q"}
        )
        print(res)
...
def submit_all(request):
    ...
    for first in range(0, len(saved_scenes), 2):
        last = first + 1
        submissions.extend(
            get_image_list(saved_scenes[first][0], saved_scenes[last][-1])
        )
...
def images(request):
    ...
    if last_scroll_id:
        try:
            response = requests.delete(
                ...
                data=json.dumps({"scroll_id": last_scroll_id}),
            )
            assert (
                response.status_code == 200
            ), f"Wrong request ({response.status_code})"
        except:
            pass
    ...
    logger.debug(
        f"{int(time.time())}, LSCBLANK, query-string, [{json.dumps(message_to_log)}], [{json.dumps(result_to_log)}]"
    )
...
def sort_by(request):
    ...
    if sortby == "time":
        queryset, scores = zip(
            *sorted(
                zip(queryset[:maximum], scores[:maximum]),
                key=lambda x: int(x[0]["scene"].split("_")[1]),
            )
        )
