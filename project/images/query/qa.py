...
qa_photo_features = np.concatenate(
    [
        qa_photo_features,
...
text_qa_model_name = (
    f"{PRETRAINED_MODELS}/models/bert-large-uncased-whole-word-masking-finetuned-squad"
)
# a) Get predictions
nlp = None
# device = "cpu"
...
def build_qa_model():
    ...
    vocab = pd.read_csv(f"{FILES_DIRECTORY}/backend/all_answers.csv")["0"].to_list()[
        :3000
    ]
    ...
    for a, aid in vocab.items():
        try:
            ...
            print(a, aid)
            raise (e)
    frozen_bilm.set_answer_embeddings(
