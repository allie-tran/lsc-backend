...
def get_embeddings(year):
    CLIP_EMBEDDINGS = os.environ.get("CLIP_EMBEDDINGS", "") + f"/{year}"
    photo_features = np.load(
        f"{CLIP_EMBEDDINGS}/{model_name}_{pretrained}_nonorm/features.npy"
    )
    photo_ids = pd.read_csv(
        f"{CLIP_EMBEDDINGS}/{model_name}_{pretrained}_nonorm/photo_ids.csv"
    )["photo_id"].to_list()
    photo_ids = [
