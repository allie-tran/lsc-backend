...
INCLUDE_FULL_SCENE = ["images", "start_time", "end_time", "gps",
                      ...
                      "ocr", "country", "location_info", "duration", "region", "date"]
...
# INDEX = "lsc2023"
# SCENE_INDEX = "lsc2023_scene_mean_vitl14"
INDEX = os.getenv("INDEX")
SCENE_INDEX = os.getenv("SCENE_INDEX")
...
def format_results(K, scene_results, embedding, group_factor, uniform, query_info):
    ...
    scene_results, scores = group_scene_results(list(zip(scene_results, scores)), group_factor, query_info)
    final_scenes = []
    if scene_results:
        final_scenes = organise_based_on_similarities(K, embedding, scene_results, cluster_scores, group_factor, uniform)
...
def query_all(query, includes, index, scroll, K, timestamp="timestamp"):
    ...
    final_scenes, scores = format_results(K, scene_results, None, group_factor="group", query_info=[])
...
def es_more(scroll_id, size=200, K=4):
    if scroll_id in ['2pairs', '3pairs']:
        ...
        final_scenes, scores = format_results(K, scene_results, cached_query.clip_embedding, group_factor="group", uniform=False, query_info=query_info)
        ...
        for key in before_query:
            if key == 'query_visualisation':
                for info_type, info_list in before_query[key].items():
                    new_info[key].extend([(info_type, "before", text) for text in info_list])
            else:
    ...
    for key in query:
        if key == 'query_visualisation':
            for info_type, info_list in query[key].items():
                new_info[key].extend([(info_type, "current", text) for text in info_list])
        else:
        ...
        for key in after_query:
            if key == 'query_visualisation':
                for info_type, info_list in after_query[key].items():
                    new_info[key].extend([(info_type, "after", text) for text in info_list])
            else:
...
def es(query, gps_bounds, size, share_info, isQuestion=False):
    ...
    if query["before"] and query["after"]:
        query, before_query, after_query, (results, scores), scroll_id = es_three_events(
            query["current"], query["before"], query["beforewhen"], query["after"], query["afterwhen"], gps_bounds, share_info, K=2, isQuestion=isQuestion)
        ...
        query, cond_query, (results, scores), scroll_id = es_two_events(
            query["current"], query[time_key], time_key, query[f"{time_key}when"], gps_bounds, share_info, K=2, isQuestion=isQuestion)
        if time_key == "before":
        ...
        query, (results, scores), scroll_id = individual_es(
            query["current"], gps_bounds, size, scroll=True, K=4, ignore_limit_score=False, cache=True, uniform=False)
...
def organise_based_on_similarities(K, embedding, scene_results, cluster_scores, group_factor, uniform):
...
def individual_es(query, gps_bounds, size, scroll, K, ignore_limit_score, cache, uniform):
    start = timecounter.time()
    ...
    return construct_es(query, gps_bounds, size, scroll, K, ignore_limit_score, cache, uniform)

def construct_es(query, gps_bounds, size, scroll, K, ignore_limit_score, cache, uniform):
    ...
    final_scenes, scores = format_results(K, scene_results, query.clip_embedding, group_factor="group", 
                                          uniform=uniform, query_info=query_info)
    return query, (final_scenes, scores), scroll_id
...
def forward_search(query, conditional_query, condition, time_limit, gps_bounds, K, reverse=False):
    ...
    query, (main_events, main_scores), _ = individual_es(
        ...
        K=K-1 if reverse else K, ignore_limit_score=False, cache=False, uniform=reverse)
    ...
    for events in conditional_events:
        ...
        cond_events, cond_scores = group_scene_results(events, group_factor="group", query_info=query_info)
        new_scenes = []
        for scene, cluster_score in zip(cond_events, cond_scores):
            ...
            scene[key] = arrange_scene(K if reverse else K-1, scene, weights, key, uniform=not reverse)            
...
def filter_pairs(pairs, condition):
    ...
    for event, main_score, cond_score in pairs:
        ...
        for e, s1, s2 in new_pairs:
            if set(e["current"]).intersection(event["current"]) and set(e[condition]).intersection(event[condition]):
                duplicated = True
                break
        if not duplicated:
...
def es_two_events(query, conditional_query, condition, time_limit, gps_bounds, share_info, K, isQuestion):
    ...
    #     max_score2 = 1
    if isQuestion:
        pair_events = sorted(pair_events, key=lambda x: query.normalise_score(x[1]) + conditional_query.normalise_score(x[2]), reverse=True)
    else:
        pair_events = sorted(pair_events, key=lambda x: query.normalise_score(x[1]) + \
                                                    0.25 * conditional_query.normalise_score(x[2]), reverse=True)

    pair_events = filter_pairs(pair_events, condition)
    print("Top paired scores:", [(query.normalise_score(x[1]), conditional_query.normalise_score(x[2]) )for x in pair_events[:10]])
...
def es_three_events(query, before, beforewhen, after, afterwhen, gps_bounds, share_info, K, isQuestion):
    global multiple_pairs
    query, before_query, * \
    ...
    if max_score3 == 0:
        max_score3 = 1
    if isQuestion:
        pair_events = sorted([(event, s1/max_score1 + s2/max_score2 * 2 + s3/max_score3 * 2)
